...
public class TestCustomizer : MonoBehaviour
{
    ...
    public void SetUpWardrobeSliders(UMA.UMAData umadata = null)
    {
        ...
        foreach (Transform child in wardrobeSlidersPanel.transform)
        {
            ...
            if (characterSystem.Recipes2[thisRace].ContainsKey(thisSlot))
            {
                child.GetComponentInChildren<Slider>().maxValue = characterSystem.Recipes2[thisRace][thisSlot].Count - 1;
                bool valueSet = false;
                if (Avatar.WardrobeRecipes2.Count > 0)
                {
                    foreach (KeyValuePair<string, UMATextRecipe> kp in Avatar.WardrobeRecipes2)
                    {
                        ...
                        {
                            child.GetComponentInChildren<Slider>().value = characterSystem.Recipes2[thisRace][recipeSlotName].FindIndex(s => s.Equals(kp.Value));
                            valueSet = true;
                        }
    ...
    public void SetUpColorSliderValue(CSColorChanger colorSlider, DynamicCharacterAvatar.ColorValue colorType)
    {
        if (sharedColorTables.FindIndex(s => s.name == colorType.Name) > -1)
        {
            ...
            for (int i = 0; i < thisColorTable.colors.Length; i++)
            {
                ...
                //if(thisColorTable.colors[i].color == colorType.Color){
                //UMA's internal function doesn't always work either
                //if(UMA.OverlayColorData.SameColor(thisColorTable.colors[i].color, colorType.Color)){
    ...
    public void SetSlot(string slotToChange, float fSlotNumber)
    {
        ...
        if (slotNumber >= 0)
        {
            UMATextRecipe tr = characterSystem.Recipes2[thisRace][slotToChange][slotNumber];
