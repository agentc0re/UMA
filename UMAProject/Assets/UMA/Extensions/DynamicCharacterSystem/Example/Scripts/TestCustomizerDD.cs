...
public class TestCustomizerDD : MonoBehaviour
{
    ...
    public void SetUpRacesDropdown(string selected = "")
    {
        ...
        //Its not the characterSystem.Recipes2.Keys its the raceLibrary
        //raceDropdownOptions = new List<string>(characterSystem.Recipes2.Keys);
    ...
    public void ChangeRace(int raceId)
    {
        ...
        if (RaceToSet != Avatar.activeRace.name)
        {
            ...
            characterSystem.context.raceLibrary.GetRace(RaceToSet);
    ...
    public void InitializeWardrobeDropDowns()
    {
        ...
        foreach (string race in characterSystem.Recipes2.Keys)
        {
            int i = 0;
            foreach (string slot in characterSystem.Recipes2[race].Keys)
            {
    ...
    public void SetUpWardrobeDropdowns()
    {
        ...
        foreach (Transform child in wardrobeDropdownPanel.transform)
        {
            ...
            if (characterSystem.Recipes2.ContainsKey(thisRace) && characterSystem.Recipes2[thisRace].ContainsKey(thisSlot) && showOption)
            {
                thisDD.options.Clear();
                thisDD.onValueChanged.RemoveAllListeners();
                var wardrobeOptions = new List<UMATextRecipe>(characterSystem.Recipes2[thisRace][thisSlot]);
                ...
                if (Avatar.WardrobeRecipes2.Count > 0)
                {
                    foreach (KeyValuePair<string, UMATextRecipe> kp in Avatar.WardrobeRecipes2)
                    {
                        var recipeSlotName = kp.Value.wardrobeSlot;
                        if (recipeSlotName == thisSlot && kp.Value.compatibleRaces.Contains(thisRace))
                        {
                            //selected = (characterSystem.Recipes2[thisRace][recipeSlotName].FindIndex(s => s.Equals(kp.Value)) + 1); Wasn't working in some cases, TODO: which cases
                            //I can do it by string I think but this will have consequences if two recipes have the same name but are effectively different...
                            for(int ri = 0; ri < characterSystem.Recipes2[thisRace][recipeSlotName].Count; ri++)
                            //foreach (UMATextRecipe r in characterSystem.Recipes2[thisRace][recipeSlotName])
                            {
                                if(characterSystem.Recipes2[thisRace][recipeSlotName][ri].name == kp.Value.name)
                                {
                        ...
                        else if (recipeSlotName == thisSlot && (Avatar.activeRace.racedata.findBackwardsCompatibleWith(kp.Value.compatibleRaces) && Avatar.activeRace.racedata.wardrobeSlots.Contains(thisSlot)))
                        {
                            ...
                            selected = (characterSystem.Recipes2[thisRace][recipeSlotName].FindIndex(s => s.Equals(kp.Value)) + 1);
                            thisDDRecipe = kp.Value;
                        }
                    }
                }
    ...
    public void SetSlot(string slotToChange, float fSlotNumber)
    {
        ...
        if (slotNumber >= 0)
        {
            tr = characterSystem.Recipes2[thisRace][slotToChange][slotNumber];
        ...
        if (prioritySlotOver.Count > 0)
        {
            ...
            //UpdateSuppressedWardrobeDropdowns(tr);
        }
    ...
    public void ListLoadableFiles(ScrollRect ItemList)
    {
        ...
        //this is where we could really do with a 'semi-singleton' version of DynamicCharacterSystem, since that already has a list of the recipes in resources...
        foreach (KeyValuePair<string,string> kp in Avatar.dynamicCharacterSystem.XMLFiles)
        {
            GameObject thisLoadableItem = Instantiate(loadableItemPrefab) as GameObject;
            thisLoadableItem.transform.SetParent(ItemList.content.transform, false);
            thisLoadableItem.GetComponent<CSLoadableItem>().customizerScript = this;
    ...
    public void LoadListedFile(string filename, string filepath = "")
    {
        ...
        {
            recipeText = Avatar.dynamicCharacterSystem.XMLFiles[filename];
        }
        if(recipeText != "")
        {
